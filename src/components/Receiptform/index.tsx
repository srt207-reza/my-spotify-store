...
const IRANIAN_BANKS = [
    "ملی ایران", "سپه", "تجارت", "ملت", "صادرات", "رسالت",
    "پارسیان", "پاسارگاد", "سامان", "سینا", "آینده", "شهر",
    "اقتصاد نوین", "کارآفرین", "دی", "ایران زمین", "خاورمیانه",
    "انصار", "مهر ایران", "توسعه صادرات", "صنعت و معدن",
    "کشاورزی", "مسکن", "پست بانک", "سایر",
];
...
export default function ReceiptForm({ orderId }: ReceiptFormProps) {
    const [payerName, setPayerName] = useState("");
    const [trackingCode, setTrackingCode] = useState("");
    const [sourceBank, setSourceBank] = useState("");
    const [loading, setLoading] = useState(false);
    const [submitted, setSubmitted] = useState(false);
    const [touched, setTouched] = useState({ payerName: false, trackingCode: false, sourceBank: false });
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch("/api/order", {
                ...
                body: JSON.stringify({
                    orderId,           // ← شناسه سفارشی که قبلاً ثبت شده
            ...
            {submitted ? (
                ...
                >
                    ...
                    <motion.div
                        ...
                        className="w-16 h-16 bg-[#1ED760]/10 border-2 border-[#1ED760]/30 rounded-full flex items-center justify-center mx-auto shadow-[0_0_24px_rgba(30,215,96,0.25)]"
                    >
                ...
                <motion.div
                    ...
                    initial="hidden"
                    animate="visible"
                    className="bg-[#181818]/80 border border-[#282828] rounded-3xl p-5 sm:p-6 space-y-5 relative overflow-hidden text-right"
                    dir="rtl"
                >
                    <motion.div
                    ...
                    <motion.div
                    //@ts-ignore
                    variants={itemVariants} className="flex items-center gap-3 relative z-10">
                    ...
                    <motion.div
                    //@ts-ignore
                    variants={itemVariants} className="relative z-10 space-y-1.5">
                        ...
                        <input
                            ...
                            className={`w-full bg-[#121212] border rounded-xl px-4 py-3 text-white placeholder:text-zinc-600 text-sm outline-none transition-all duration-200
                                ${touched.payerName && !payerNameValid ? "border-red-500/60" : payerNameValid ? "border-[#1ED760]/40 focus:border-[#1ED760]" : "border-[#282828] focus:border-zinc-500"}`}
                        />
                        {touched.payerName && !payerNameValid && (
                            <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-red-400 text-xs">الزامی</motion.p>
                        )}
                    </motion.div>
                    ...
                    variants={itemVariants} className="relative z-10 space-y-1.5">
                        ...
                        <input
                            ...
                            className={`w-full bg-[#121212] border rounded-xl px-4 py-3 text-white placeholder:text-zinc-600 text-sm outline-none transition-all duration-200 text-right
                                ${touched.trackingCode && !trackingCodeValid ? "border-red-500/60" : trackingCodeValid ? "border-[#1ED760]/40 focus:border-[#1ED760]" : "border-[#282828] focus:border-zinc-500"}`}
                        />
                        {touched.trackingCode && !trackingCodeValid && (
                            <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-red-400 text-xs">الزامی (حداقل ۶ رقم)</motion.p>
                        )}
                    </motion.div>

                    <motion.div 
                    //@ts-ignore
                    variants={itemVariants} className="relative z-20 space-y-1.5">
                        <label className="text-zinc-300 text-sm font-medium flex items-center gap-1">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className={`w-full bg-[#121212] border rounded-xl px-4 py-3 text-sm outline-none transition-all duration-200 flex items-center justify-between
                                    ${touched.sourceBank && !sourceBankValid ? "border-red-500/60" : sourceBankValid ? "border-[#1ED760]/40" : "border-[#282828]"}
                                            ...
                                            <li key={bank}>
                                                <button
                                                    ...
                                                    className={`w-full text-right px-4 py-2.5 text-sm transition-colors
                                                        ${sourceBank === bank ? "text-[#1ED760] bg-[#1ED760]/10" : "text-zinc-300 hover:bg-[#282828] hover:text-white"}`}
                                                >
                    ...
                    <motion.div
                    //@ts-ignore
                    variants={itemVariants} className="relative z-10 pt-1">
                        <motion.button
                        ...
                        >
                            ...
                            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <span className="relative z-10">تأیید و ثبت رسید پرداخت</span>}
                        </motion.button>
                    </motion.div>
                </motion.div>
            )}
